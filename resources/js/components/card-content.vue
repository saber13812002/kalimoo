<template>
    <div class="col-xs col-sm col- col-md col-lg col-xl-12 content-card margin-top-fixd">
        <div class="col-xs col-sm col- col-md col-lg col-xl-12  buy-card flex">
            ...
            <card-pay-right @factor="get_factor($event)" :sending_cost="sending_cost" v-if="ok === 3" @cost-after-off="getNewPrice($event)" @send-pay-method="payMethod($event)" :price="price" :orders1="orders" @final-cost="get2($event)"></card-pay-right>
            <card-left :factor="factor" :sending_cost="sending_cost" :minimum_price="minimum_price" :P_method="P_method" :user="user" :price="price" :off="off" :orders1="orders" :check="check"></card-left>
        </div>
    </div>
</template>
